...
Capture validates the four trust-zone names and explicitly enforces the three allowed sensitivity tiers: `public_safe`, `normal` (default), and `do_not_export` (rejecting unknown tiers with `invalid_sensitivity_tier`). An unknown retrieval zone fails closed explicitly with an `invalid_trust_zone` receipt status.
...
The adapter test proves directory initialization, safe title slugging, frontmatter injection neutralization, roundtrip reads, same-title collision disambiguation, and rejection of path traversal outside the configured wiki root. It does not prove semantic lifecycle policy or retrieval/export enforcement.
