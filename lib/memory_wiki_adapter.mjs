...
export class MemoryWikiAdapter {
  ...
  _assertInsideBaseDir(candidatePath) {
    ...
    // Walk up the directory tree to find the first existing parent to check for symlink escapes
    let pathToCheck = resolved;
    while (!fs.existsSync(pathToCheck) && pathToCheck !== path.dirname(pathToCheck)) {
      pathToCheck = path.dirname(pathToCheck);
    }

    if (fs.existsSync(pathToCheck)) {
      try {
        const real = fs.realpathSync(pathToCheck);
        const realBase = fs.realpathSync(this.baseDir);
        const realRelative = path.relative(realBase, real);
        if (realRelative.startsWith("..") || path.isAbsolute(realRelative)) {
          throw new Error("Memory wiki path escapes the configured wiki root via symlink.");
        }
      } catch (err) {
        if (err.message && err.message.includes("escapes")) throw err;
      }
    }
    return resolved;
  }
  ...
  writeMemory(memory) {
    ...
    const baseSafeName = String(memory.title || memory.memory_id || "unknown")
      .replace(/[^a-z0-9-]/gi, "-")
      .replace(/-+/g, "-")
      .replace(/^-|-$/g, "")
      .toLowerCase() || "unknown";

    let fileName = `${baseSafeName}.md`;
    let filePath = this._assertInsideBaseDir(path.join(this.baseDir, folder, fileName));

    if (fs.existsSync(filePath)) {
      const existing = this.readMemory(filePath);
      const incomingId = String(memory.memory_id || "").trim();
      const existingId = String(existing?.memory_id || "").trim();
      if (existing && existingId && incomingId && existingId !== incomingId) {
        const idSuffix = incomingId.replace(/[^a-z0-9-]/gi, "").slice(-8) || "collision";
        fileName = `${baseSafeName}--${idSuffix}.md`;
        filePath = this._assertInsideBaseDir(path.join(this.baseDir, folder, fileName));
      }
    }
