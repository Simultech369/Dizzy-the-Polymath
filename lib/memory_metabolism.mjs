...
import { validateTrajectoryDistillationContract } from "./trajectories.mjs";
...
export function summarizeMemoryMetabolism(opts = {}) {
  ...
  for (const row of rows) {
    ...
    try {
      validateTrajectoryDistillationContract(parsed.distillation_contract || {});
    } catch (err) {
      findings.push({
        severity: "yellow",
        kind: "invalid_distillation_contract",
        id,
        message: `${id} has invalid distillation contract: ${String(err?.message || err)}`,
      });
    }
