...
function normalizeEffectiveThresholds(thresholds = DEFAULT_THRESHOLDS) {
...
function hasFailureSignal(value) {
  ...
  if (exitCode !== undefined && exitCode !== null && Number(exitCode) !== 0) return true;
  if (value.ok === false || value.success === false) return true;
  const stderr = typeof value.stderr === 'string' ? value.stderr : '';
  const error = typeof value.error === 'string' ? value.error : '';
  const exception = typeof value.exception === 'string' ? value.exception : '';
  return /fatal|error|failed|failure|exception|traceback/i.test([stderr, error, exception].join('\n'));
}
