...
const FAILURE_TEXT_RE = /fatal|error|failed|failure|exception|traceback|denied|eacces|eperm|unauthorized|forbidden/i;
...
function hasNonZeroExitAlias(value) {
  ...
  for (const key of ['exit_code', 'exitCode', 'code']) {
    ...
    if (typeof raw === 'string') {
      ...
      } else if (key === 'code' && FAILURE_TEXT_RE.test(trimmed)) {
        return true;
      }
    }
  }
