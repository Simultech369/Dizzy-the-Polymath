...
export function getRuntimeSafetyConfig() {
  const authToken = String(env("DIZZY_AUTH_TOKEN", "")).trim();
  return {
    ...
    verifiedHttps: parseBool("DIZZY_VERIFIED_HTTPS", false),
  };
}
...
export function validateRuntimeSafetyConfig(config = getRuntimeSafetyConfig()) {
  ...
  const authTokenLength = Number(config.authTokenLength ?? 0);

  if (config.verifiedHttps && deploymentMode === "direct_local") {
    errors.push("DIZZY_VERIFIED_HTTPS=1 requires DIZZY_DEPLOYMENT_MODE to be proxied or hosted.");
  }
  ...
  if (!["", "gemini", "openai_compat", "openrouter", "local"].includes(config.chatBackend)) {
    errors.push(`Invalid DIZZY_CHAT_BACKEND='${config.chatBackend}'. Use gemini, openai_compat, or local.`);
  }
  ...
  if (["proxied", "hosted"].includes(deploymentMode) && config.authTokenConfigured && authTokenLength < 32) {
    errors.push(`DIZZY_AUTH_TOKEN must be at least 32 characters when DIZZY_DEPLOYMENT_MODE=${deploymentMode}.`);
  }
