...
function safeErrorToken(value, fallback = "ERROR") {
  const raw = String(value || fallback).trim();
  const token = raw.replace(/[^A-Za-z0-9_-]+/g, "_").replace(/^_+|_+$/g, "").slice(0, 80);
  return token || fallback;
}

function formatPersistedJobError(err, retryReason) {
  const code = safeErrorToken(err?.code || err?.name || retryReason || "JOB_FAILED", "JOB_FAILED");
  const status = Number(err?.status ?? err?.statusCode ?? 0) || 0;
  const parts = [`JOB_FAILED code=${code}`];
  if (status > 0) parts.push(`status=${status}`);
  if (retryReason) parts.push(`reason=${safeErrorToken(retryReason, "retry")}`);
  return parts.join(" ");
}
...
export async function runWorkerCycle(redis, keys, runJob, opts = {}) {
  ...
  try {
    ...
    const errMsg = formatPersistedJobError(err, klass.reason);
