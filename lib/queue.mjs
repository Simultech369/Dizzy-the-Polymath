...
export async function acknowledgeNotifications(redis, key, receipts) {
  ...
  const script = `
    ...
    local items = redis.call('LRANGE', KEYS[1], 0, #expected - 1)
    if #items ~= #expected then return -1 end
    for i = 1, #expected do
      if redis.sha1hex(items[i]) ~= expected[i] then return -2 end
    end
    redis.call('LTRIM', KEYS[1], #expected, -1)
    return #expected
  `;
