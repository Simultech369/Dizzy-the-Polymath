  ...
  for (const id of ids || []) {
    ...
    if (!job || job.status === "succeeded" || job.status === "dead") {
      if (job && job.status === "dead") {
        // Recover missing DLQ/notification side effects. If Redis fails after the side effect
        // but before the marker write, a later recovery may repeat it; after markers persist,
        // repeated recovery skips the side effects.
        if (!job.dlq_enqueued_at_ms) {
          await pushDlq(redis, keys, id);
          await markJob(redis, keys, id, { dlq_enqueued_at_ms: String(nowMs()) });
        }
        if (!job.death_notified_at_ms) {
          try {
            await notifyJobDeath(redis, keys, job, { recovered_after_worker_crash: true });
            await markJob(redis, keys, id, { death_notified_at_ms: String(nowMs()) });
          } catch {
            // best effort
          }
        }
...
export async function runWorkerCycle(redis, keys, runJob, opts = {}) {
  ...
  try {
    ...
    if (!klass.retry || !retryAllowed || outOfAttempts || outOfRetries) {
      ...
      await markJob(redis, keys, jobId, { dlq_enqueued_at_ms: String(nowMs()) });
      await acknowledgeClaim(redis, keys, jobId);
      ...
      try {
        ...
        if (jobAfter) {
          ...
          await markJob(redis, keys, jobId, { death_notified_at_ms: String(nowMs()) });
        }
      } catch {
