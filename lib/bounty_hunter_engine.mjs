...
export const VALID_BOUNTY_ELIGIBILITY_STATES = Object.freeze([
  "needs_verification",
  "eligible",
  "benchmark",
]);
...
function normalizeEligibilityState(value) {
  const state = normalizeSlug(value, "needs_verification");
  return VALID_BOUNTY_ELIGIBILITY_STATES.includes(state) ? state : "needs_verification";
}

function hasExplicitReproductionEvidence(proofRequirements) {
  return cleanList(proofRequirements).some((entry) => /reproduction|regression|fixture|proof/i.test(entry));
}

export function assessBountyEligibility({
  ...
  const explicitEvidence = hasExplicitReproductionEvidence(proofRequirements);
  const payout = Number.isFinite(Number(payoutUsd)) ? Math.max(0, Number(payoutUsd)) : 0;
  const benchmarkSelected = Boolean(benchmarkMode) && payout === 0;
  const claimabilityEligible = safeClaimabilityState === "open_unassigned" || safeClaimabilityState === "open_needs_assignment";

  if (benchmarkSelected) {
    return {
      eligibility_state: "benchmark",
      claimability_state: safeClaimabilityState,
      payout_usd: payout,
      has_reproduction_tests: explicitEvidence,
      dispatchable: false,
      reason: "Benchmark mode was explicitly selected for a zero-payout opportunity",
    };
  }

  if (claimabilityEligible && payout > 0 && explicitEvidence) {
    return {
      eligibility_state: "eligible",
      claimability_state: safeClaimabilityState,
      payout_usd: payout,
      has_reproduction_tests: true,
      dispatchable: true,
      reason: "Claimability, payout, and reproduction evidence are all explicit",
    };
  }

  const reasonParts = [];
  if (!claimabilityEligible) reasonParts.push(`claimability '${safeClaimabilityState}' is not open for direct routing`);
  if (payout <= 0) reasonParts.push("payout is not explicit and positive");
  if (!explicitEvidence) reasonParts.push("no explicit reproduction evidence was supplied");

  return {
    eligibility_state: normalizeEligibilityState("needs_verification"),
    claimability_state: safeClaimabilityState,
    payout_usd: payout,
    has_reproduction_tests: explicitEvidence,
    dispatchable: false,
...
export function calculateBountyEv({
  ...
  else if (payout === 0) recommendation = benchmarkMode ? "EVAL_BENCHMARK_ONLY" : "NEEDS_VERIFICATION";

  const evRatio = estimatedCost > 0 ? Number((evUsd / estimatedCost).toFixed(2)) : (evUsd > 0 ? 999.99 : 0);
  ...
  return {
    ...
    benchmark_mode: Boolean(benchmarkMode),
...
export async function processBountyIngestJob(jobPayload, {
  ...
  // 2. Perform EV triage from the task itself.
  const rawTriageReceipt = calculateBountyEv({
    payoutUsd: bountyTask.payout_usd,
    difficulty: bountyTask.difficulty,
    hasReproductionTests: hasExplicitReproductionEvidence(bountyTask.proof_requirements),
    benchmarkMode,
  });

  const eligibility = assessBountyEligibility({
    claimabilityState: bountyTask.claimability_state,
    payoutUsd: bountyTask.payout_usd,
    proofRequirements: bountyTask.proof_requirements,
    benchmarkMode,
  });

  const triageReceipt = {
    ...rawTriageReceipt,
    recommendation: eligibility.dispatchable
      ? rawTriageReceipt.recommendation
      : eligibility.eligibility_state === "benchmark"
        ? "EVAL_BENCHMARK_ONLY"
        : rawTriageReceipt.recommendation === "REJECT_NEGATIVE_EV"
          ? rawTriageReceipt.recommendation
          : "NEEDS_VERIFICATION",
  };

  if (!eligibility.dispatchable) {
    const routeStatus = triageReceipt.recommendation === "REJECT_NEGATIVE_EV"
      ? "REJECTED"
      : eligibility.eligibility_state === "benchmark"
        ? "BENCHMARK_ONLY"
        : "NEEDS_VERIFICATION";
    return {
      ...
      status: routeStatus,
      reason: routeStatus === "REJECTED"
        ? `Triage recommendation '${triageReceipt.recommendation}' prevents automated dispatch`
        : eligibility.reason || `Triage recommendation '${triageReceipt.recommendation}' prevents automated dispatch`,
      triage_receipt: triageReceipt,
      runbook: null,
      fsm_receipt: null,
