...
export function appendTrajectory(input, opts = {}) {
  ...
  const admissionSource = {
    schema_version: "dizzy.trajectory_admission_source.v1",
...
function validateAdmissionSource(parsed, normalized, receipt) {
  ...
  if (source.schema_version !== "dizzy.trajectory_admission_source.v1") return false;
  if (!Object.prototype.hasOwnProperty.call(source, "evidence")) return false;
  if (!source.effective_policy || typeof source.effective_policy !== "object" || Array.isArray(source.effective_policy)) return false;
  if (!Array.isArray(receipt.results) || receipt.results.length !== 1) return false;
  const result = receipt.results[0];
  if (!result || typeof result !== "object") return false;
  if (hashTrajectoryEvidence(source.evidence) !== result.input_evidence_sha256) return false;
  if (hashTrajectoryEvidence([source.evidence]) !== receipt.input_evidence_sha256) return false;
  if (hashTrajectoryEvidence(source.effective_policy) !== receipt.policy_sha256) return false;
  if (source.evidence_sha256 !== result.input_evidence_sha256) return false;
  if (source.batch_input_evidence_sha256 !== receipt.input_evidence_sha256) return false;
  if (source.policy_sha256 !== receipt.policy_sha256) return false;
  if (source.admission_receipt_sha256 !== receipt.receipt_sha256) return false;
  if (source.normalized_record_sha256 !== hashTrajectoryRecord(normalized)) return false;

  const replay = evaluateBatch([source.evidence], source.effective_policy);
  if (replay.overall_status !== "TRAJECTORY_SUITE_PASSED") return false;
  if (replay.total_trajectories !== 1 || replay.passed_trajectories !== 1 || replay.failed_trajectories !== 0) return false;
  if (replay.total_violations !== 0) return false;
  if (replay.policy_sha256 !== receipt.policy_sha256) return false;
  if (replay.input_evidence_sha256 !== receipt.input_evidence_sha256) return false;
  if (!Array.isArray(replay.results) || replay.results.length !== 1 || replay.results[0].status !== "PASSED") return false;
  return true;
}
...
export function readTrajectories(opts = {}) {
  ...
  for (const line of lines) {
    try {
      ...
      if (!parsed.admission_evidence || typeof parsed.admission_evidence !== "object") continue;
      if (!parsed.admission_receipt || typeof parsed.admission_receipt !== "object") continue;
      ...
      if (requiredEvidence.some((key) => typeof parsed.admission_evidence[key] !== "string" || !parsed.admission_evidence[key].trim())) {
        continue;
      }
      if (requiredEvidence.some((key) => !/^[a-f0-9]{64}$/i.test(parsed.admission_evidence[key]) && key !== "evaluator_revision")) {
        continue;
      }
      ...
      const normalized = normalizeTrajectory(parsed, { now: new Date(parsed.timestamp || Date.now()) });
      if (parsed.admission_evidence.normalized_record_sha256 !== hashTrajectoryRecord(normalized)) {
        continue;
      }
      if (!validateAdmissionSource(parsed, normalized, parsed.admission_receipt)) {
        continue;
      }
