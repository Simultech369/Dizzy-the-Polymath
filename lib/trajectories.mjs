...
function summarizeStepAction(step, index) {
  ...
  const exitCode = step.exit_code ?? step.exitCode ?? step.code;
  const stderr = normalizeString(step.stderr ?? step.error ?? step.exception, 160);
  const parts = [
    status ? `status=${status}` : "",
    actor ? `actor=${actor}` : "",
    tool ? `tool=${tool}` : "",
    exitCode !== undefined && exitCode !== null ? `exit_code=${normalizeString(exitCode, 20)}` : "",
...
export function readTrajectories(opts = {}) {
  ...
  for (const line of lines) {
    try {
      ...
      if (
        parsed.admission_evidence.admission_receipt_sha256
        && parsed.admission_evidence.admission_receipt_sha256 !== parsed.admission_receipt.receipt_sha256
      ) {
        continue;
      }
      const normalized = normalizeTrajectory(parsed, { now: new Date(parsed.timestamp || Date.now()) });
      if (
        parsed.admission_evidence.normalized_record_sha256
        && parsed.admission_evidence.normalized_record_sha256 !== hashTrajectoryRecord(normalized)
      ) {
        continue;
      }
