...
import { resolvePromptVersion } from "./prompt_registry.mjs";
...
export function getPromptSources(options = {}) {
  ...
  for (const absPath of sortedFiles) {
    ...
    const registryRes = resolvePromptVersion(absPath, "prompt_registry.json", process.env.DIZZY_PROMPT_VERSION);
    const resolvedAbsPath = registryRes.path;

    const raw = readFileText(resolvedAbsPath);
    if (!raw) {
      sources.push({
        path: rel,
        ...
        text: "",
        is_sink: isSink,
        registry_version: registryRes.version,
        registry_experiment: registryRes.experiment,
      });
      continue;
    }
