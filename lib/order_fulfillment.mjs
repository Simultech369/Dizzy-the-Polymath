...
export function hasUnresolvedExternalEffect({ pendingRecords = [], completionRecords = [], operation, referenceField, referenceKey }) {
  const key = String(referenceKey || "");
  if (!key) return false;
  const completed = completionRecords.some((record) => String(record?.payload?.[referenceField] || "") === key);
  if (completed) return false;
  return pendingRecords.some((record) => record?.payload?.operation === operation
    && String(record?.payload?.[referenceField] || "") === key);
}
