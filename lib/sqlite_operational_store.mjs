/**
 * [STATUS: EXPERIMENTAL / SIDECAR]
 * Authority: Retained as a local sidecar. Reversible and non-authoritative.
 * Replaced by the Cognitive Memory Engine graph adapter for production receipts.
 */
import fs from "fs";
...
export function openOperationalStore(filePath, opts = {}) {
  ...
  try {
    ...
    CREATE TABLE IF NOT EXISTS replay_leases (
  ...
  function acquireReplayLease({ leaseKey, owner, leaseMs = 60000, idempotencyKey = "", operationFingerprint = "" }) {
    const key = normalizeKey(leaseKey, "replay_unknown");
    const normalizedOwner = normalizeKey(owner, "owner_unknown");
    const normalizedIdempotencyKey = String(idempotencyKey || "").trim();
    const fingerprint = String(operationFingerprint || JSON.stringify({ operation: "replayLease", lease_key: key }));
    const now = nowIso();
    const nowMs = Date.parse(now);
    const expiresAt = new Date(Date.now() + Math.max(1, Number(leaseMs) || 60000)).toISOString();

    return transaction(() => {
      const rowByIdempotency = normalizedIdempotencyKey
        ? db.prepare("SELECT lease_key FROM replay_leases WHERE idempotency_key=?").get(normalizedIdempotencyKey)
        : null;
      if (rowByIdempotency && rowByIdempotency.lease_key !== key) {
        throw new Error(`Replay idempotency key conflict: ${normalizedIdempotencyKey}`);
      }

      const existing = db.prepare("SELECT * FROM replay_leases WHERE lease_key=?").get(key);
      if (existing) {
        if (existing.operation_fingerprint !== fingerprint) {
          throw new Error(`Replay lease request conflict: ${key}`);
        }
        if (existing.idempotency_key && normalizedIdempotencyKey && existing.idempotency_key !== normalizedIdempotencyKey) {
          throw new Error(`Replay idempotency key conflict: ${normalizedIdempotencyKey}`);
        }
        ...
        if (existing.status === "leased" && Number.isFinite(leaseExpiryMs) && leaseExpiryMs > nowMs) {
          return {
            acquired: false,
            replay: false,
            lease_key: key,
            owner: existing.owner,
            status: "leased",
            retry_after_ms: Math.max(0, leaseExpiryMs - nowMs),
          };
        }
        db.prepare(`
          UPDATE replay_leases
          SET owner=?, idempotency_key=COALESCE(idempotency_key, ?), status='leased',
              lease_expires_at=?, updated_at=?
          WHERE lease_key=?
        `).run(normalizedOwner, normalizedIdempotencyKey || null, expiresAt, now, key);
        return {
          acquired: true,
          replay: false,
          lease_key: key,
      ...
      db.prepare(`
        INSERT INTO replay_leases(lease_key, owner, idempotency_key, status, operation_fingerprint, result_fingerprint, lease_expires_at, created_at, updated_at)
        VALUES (?, ?, ?, 'leased', ?, NULL, ?, ?, ?)
      `).run(key, normalizedOwner, normalizedIdempotencyKey || null, fingerprint, expiresAt, now, now);
  ...
  function completeReplayLease({ leaseKey, owner, resultFingerprint = "" }) {
    const key = normalizeKey(leaseKey, "replay_unknown");
    const normalizedOwner = normalizeKey(owner, "owner_unknown");
    const fingerprint = String(resultFingerprint || "");
    const now = nowIso();

    return transaction(() => {
      const existing = db.prepare("SELECT * FROM replay_leases WHERE lease_key=?").get(key);
      if (!existing) throw new Error(`Replay lease not found: ${key}`);
      if (existing.owner !== normalizedOwner) {
        throw new Error(`Replay lease owner conflict: ${key}`);
      }
      if (existing.status === "completed") {
        return { lease_key: key, owner: normalizedOwner, status: "completed", replay: true };
      }
      if (existing.status !== "leased") {
        throw new Error(`Replay lease is not active: ${key}`);
      }
      db.prepare(`
        UPDATE replay_leases
        SET status='completed', result_fingerprint=?, updated_at=?
        WHERE lease_key=? AND owner=? AND status='leased'
      `).run(fingerprint || null, now, key, normalizedOwner);
      return { lease_key: key, owner: normalizedOwner, status: "completed", replay: false };
    });
  }

  function failReplayLease({ leaseKey, owner, resultFingerprint = "" }) {
    const key = normalizeKey(leaseKey, "replay_unknown");
    const normalizedOwner = normalizeKey(owner, "owner_unknown");
    const fingerprint = String(resultFingerprint || "");
    const now = nowIso();
    ...
    return transaction(() => {
      ...
      db.prepare(`
        UPDATE replay_leases
        SET status='failed', result_fingerprint=?, updated_at=?
        WHERE lease_key=? AND owner=? AND status='leased'
      `).run(fingerprint || null, now, key, normalizedOwner);
      return { lease_key: key, owner: normalizedOwner, status: "failed" };
    });
  }
  ...
  function getReplayLease(leaseKey) {
    return db.prepare("SELECT * FROM replay_leases WHERE lease_key=?").get(normalizeKey(leaseKey, "replay_unknown")) ?? null;
  }
