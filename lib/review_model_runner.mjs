...
export const MODEL_EXECUTION_PROFILES = Object.freeze({
  ...
  "deepseek-r1:7b": {
    review_usable: true,
    json_reliability: "reasoning_adapter_enabled",
    expected_latency_band: "local_cpu_very_slow",
...
export function stripReasoningContent(text = "") {
  ...
  if (raw.toLowerCase().includes("<think>")) {
    const idx = raw.toLowerCase().indexOf("<think>");
    const after = raw.slice(idx + 7);
    if (after.toLowerCase().includes("</think>")) {
      const endIdx = after.toLowerCase().indexOf("</think>");
      raw = (raw.slice(0, idx) + after.slice(endIdx + 8)).trim();
    } else {
...
function extractJsonObject(text) {
  const raw = stripReasoningContent(text);
...
export function resolveReviewerExecutionTarget(reviewer = {}, {
  ...
  if (reviewer.execution_target && typeof reviewer.execution_target === "object") {
    const target = reviewer.execution_target;
    const model = String(target.model || target.model_id || reviewer.primary_model || "").trim();
    const baseUrl = String(target.baseUrl || target.base_url || "").trim();
    return {
      executable: target.executable !== false,
      backend: target.backend || "openai_compat",
      route: target.route || { backend: target.backend || "openai_compat", primary_model: model },
      baseUrl,
      apiKey: target.apiKey || target.api_key || "",
      model,
      model_profile: target.model_profile || getModelExecutionProfile(model),
      selected_reason: target.selected_reason || "operator_resolved_review_selection",
      original_model: target.original_model || reviewer.primary_model || model,
      isLocalIsolationRequired: target.isLocalIsolationRequired !== false,
      base_url_host: target.base_url_host || safeHost(baseUrl),
    };
  }
  ...
  if (remote && trustZone === "private_self" && !isOllama) {
    return { executable: false, skipped_reason: "private_zone_cloud_disallowed", route, model, model_profile: getModelExecutionProfile(model), backend: "openai_compat", baseUrl, base_url_host: safeHost(baseUrl), isLocalIsolationRequired: true };
  }
...
export async function executeReviewerModelReview({
  ...
  const target = resolveReviewerExecutionTarget(reviewer, { allowCloud, trustZone, preferLocalFallbacks, cloudProvider, cloudBaseUrl, cloudApiKey, allowProviderEnv });
  const packet = buildReviewerPacket({ plan, reviewer, diffText, maxDiffChars, allowCloud, trustZone, preferLocalFallbacks, cloudProvider, cloudBaseUrl, cloudApiKey, allowProviderEnv, reviewProfile, maxFindings });
