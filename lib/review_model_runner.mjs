...
export function resolveReviewerExecutionTarget(reviewer = {}, {
  ...
  if (!baseUrl) {
    return { executable: false, skipped_reason: "openai_compat_base_url_missing", route, model };
  }
  const remote = isRemoteCloudBackend("openai_compat", baseUrl);
  if ((remote || trustZone === "private_self") && !allowCloud && !isOllama) {
    return { executable: false, skipped_reason: "cloud_review_blocked_without_allow_cloud", route, model, base_url_host: safeHost(baseUrl) };
  }
  if (!isOllama && !apiKey) {
    return { executable: false, skipped_reason: "openai_compat_api_key_missing", route, model, base_url_host: safeHost(baseUrl) };
  }
...
export function buildReviewerPacket({
  ...
  const target = resolveReviewerExecutionTarget(reviewer, { allowCloud, trustZone });
  const systemPrompt = [
...
export function buildModelReviewPackets(plan, {
  ...
  return (Array.isArray(plan?.reviewer_assignments) ? plan.reviewer_assignments : [])
    .map((reviewer) => buildReviewerPacket({ plan, reviewer, diffText, maxDiffChars, allowCloud, trustZone }));
}
...
export async function executeReviewerModelReview({
  ...
  const target = resolveReviewerExecutionTarget(reviewer, { allowCloud, trustZone });
  const packet = buildReviewerPacket({ plan, reviewer, diffText, allowCloud, trustZone });
  ...
  try {
    ...
    const parsed = parseReviewerResponseText(text, reviewer);
    return {
      ...parsed,
      target: {
...
export async function runModelReviewBatch({
  ...
  const packets = buildModelReviewPackets(plan, { diffText, allowCloud, trustZone });
  const reviews = [];
  if (executeModels) {
    for (const reviewer of Array.isArray(plan.reviewer_assignments) ? plan.reviewer_assignments : []) {
      reviews.push(await executeReviewerModelReview({
        plan,
        reviewer,
        diffText,
        allowCloud,
