...
function localFastInstructionLines({ maxFindings = 2 } = {}) {
  const limit = Math.max(1, Number(maxFindings) || 2);
  return [
    "Local-fast profile: return one compact JSON object and no markdown fence.",
...
export function buildReviewerPacket({
  ...
  const profileLines = reviewProfile === "local_fast" ? localFastInstructionLines({ maxFindings }) : [];
  const systemPrompt = [
...
export function buildModelReviewPackets(plan, {
  ...
  return (Array.isArray(plan?.reviewer_assignments) ? plan.reviewer_assignments : [])
    .map((reviewer) => buildReviewerPacket({ plan, reviewer, diffText, maxDiffChars, allowCloud, trustZone, preferLocalFallbacks, reviewProfile, maxFindings }));
}
...
export async function executeReviewerModelReview({
  ...
  const target = resolveReviewerExecutionTarget(reviewer, { allowCloud, trustZone, preferLocalFallbacks });
  const packet = buildReviewerPacket({ plan, reviewer, diffText, allowCloud, trustZone, preferLocalFallbacks, reviewProfile, maxFindings });
  ...
  try {
    const text = await generateText({
      ...
      temperature,
      maxTokens,
      responseFormat: reviewProfile === "local_fast" ? { type: "json_object" } : undefined,
...
export async function runModelReviewBatch({
  ...
  const packets = buildModelReviewPackets(plan, { diffText, allowCloud, trustZone, preferLocalFallbacks, reviewProfile, maxFindings });
