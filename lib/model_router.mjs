...
export function normalizeCouncilSelection(selection = {}) {
  ...
  return {
    seat_id: normalizeId(src.seat_id || src.seat || ""),
    model_id: String(src.model_id || src.model || "").trim(),
    harness_id: normalizeId(src.harness_id || src.harness || "native_chat", "native_chat"),
  };
}
...
export function resolveCouncilSelection(selection = {}) {
  ...
  const seat = EXECUTABLE_LOCAL_SEATS[normalized.seat_id]
  ...
  return {
    ...
    api_key: "local_nop",
    route_id: routeId,
    provider_boundary: classifiedBaseUrl.isLocalHost ? "local_machine" : "private_lan",
    seat_class: seat.seat_class,
    evidence_state: "configured_unverified",
    authority: "operator_requested_not_availability_proof",
  };
