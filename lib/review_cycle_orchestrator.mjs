...
function reviewerHistoryScore(reviewer, history = {}) {
  ...
  const attempts = Math.max(runs + skipped + failures, Number(stats.attempts || 0));
  ...
  const skippedRate = attempts ? skipped / attempts : 0;
  const failureRate = attempts ? failures / attempts : 0;
  const novelty = runs === 0 ? 4 : Math.max(0, 2 - Math.log2(runs + 1) * 0.35);
  return novelty + confirmedRate * 4 + disagreementRate * 2 - falsePositiveRate * 2 - skippedRate * 3 - failureRate * 4;
}
...
export function reconcileReviewBatch({
  ...
  const usableReviews = normalizedReviews.filter((review) => !["skipped", "failed", "errored"].includes(String(review.status || "").toLowerCase()));
  ...
  const findings = usableReviews.flatMap((review) => Array.isArray(review.findings) ? review.findings.map((finding) => ({ ...finding, source: review.source })) : []);
  ...
  } else if (usableReviews.length >= minReviewsForPush && (!requireDisagreement || usefulDisagreements.length > 0)) {
  ...
  return {
    ...
    reason,
    counts: {
      reviews: usableReviews.length,
      skipped_reviews: normalizedReviews.length - usableReviews.length,
...
export function updateReviewHistory(history = {}, reviews = []) {
  ...
  for (const review of Array.isArray(reviews) ? reviews : []) {
    ...
    const status = String(review.status || "submitted").toLowerCase();
    const usable = !["skipped", "failed", "errored"].includes(status);
    const findings = usable && Array.isArray(review.findings) ? review.findings : [];
    ...
    const priorAttempts = Math.max(
      ...
      Number(current.runs || 0) + Number(current.skipped || 0) + Number(current.failures || 0),
    );
    next.reviewers[key] = {
      attempts: priorAttempts + 1,
      runs: Math.max(0, Number(current.runs || 0)) + (usable ? 1 : 0),
      skipped: Math.max(0, Number(current.skipped || 0)) + (status === "skipped" ? 1 : 0),
      failures: Math.max(0, Number(current.failures || 0)) + (["failed", "errored"].includes(status) ? 1 : 0),
      ...
      last_status: status || "submitted",
      last_skipped_reason: String(review.skipped_reason || current.last_skipped_reason || "").slice(0, 160),
      last_likely_root_cause: String(review.diagnosis?.likely_root_cause || current.last_likely_root_cause || "").slice(0, 160),
    };
  }
  return next;
}
