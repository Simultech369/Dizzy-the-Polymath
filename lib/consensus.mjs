...
export function saveConsensusState(state) {
  try {
    ...
    try {
      ...
      if (renameErr.code === "EPERM" || renameErr.code === "EBUSY" || renameErr.code === "EXDEV") {
...
export function signOffOperator() {
  ...
  const saved = saveConsensusState(state);
  if (!saved) {
    return {
      ok: false,
      error: "CONSENSUS_STATE_SAVE_FAILED",
      message: "Simulated operator sign-off was not recorded because consensus state persistence failed.",
  ...
  return {
    ok: true,
    message: "Simulated operator sign-off recorded. Consensus reached in the local rehearsal state.",
    signing_chain: state.signing_chain,
    consensus_status: state.consensus_status,
    basis: state.basis || "operator_reported_review_state",
    proof_limit: state.proof_limit || "not_cryptographic_not_live_multi_agent_protocol",
...
export function vetoOperator() {
  ...
  if (!saved) {
    return {
      ok: false,
      error: "CONSENSUS_STATE_SAVE_FAILED",
      message: "Simulated operator veto was not recorded because consensus state persistence failed.",
      signing_chain: state.signing_chain,
      consensus_status: state.consensus_status,
      basis: state.basis || "operator_reported_review_state",
      proof_limit: state.proof_limit || "not_cryptographic_not_live_multi_agent_protocol",
  ...
  return {
    ok: true,
    message: "Simulated operator veto recorded in the local rehearsal state.",
    signing_chain: state.signing_chain,
    consensus_status: state.consensus_status,
    basis: state.basis || "operator_reported_review_state",
    proof_limit: state.proof_limit || "not_cryptographic_not_live_multi_agent_protocol",
...
export function initializeNewProposal(options = DEFAULT_OPTIONS) {
  ...
  const saved = saveConsensusState(newState);
  if (!saved) {
    return {
      ...newState,
      ok: false,
      error: "CONSENSUS_STATE_SAVE_FAILED",
      message: "Simulated proposal state was not initialized because consensus state persistence failed.",
    };
  }
  return newState;
}
