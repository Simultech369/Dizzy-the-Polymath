...
import { runWithToolLifecycle } from "./lifecycle_hooks.mjs";
...
function parseMappedIpv4(host) {
  ...
  const numericGroups = groups?.map((group) => parseInt(group, 16));
  if (numericGroups &&
      numericGroups.every((group) => Number.isInteger(group) && group >= 0 && group <= 0xffff) &&
      numericGroups[0] === 0 && numericGroups[1] === 0 && numericGroups[2] === 0 &&
      numericGroups[3] === 0 && numericGroups[4] === 0 && numericGroups[5] === 0xffff) {
...
export async function validateExternalUrl(url, opts = {}) {
  ...
  const requestedDnsTimeoutMs = Number(opts.dnsTimeoutMs);
  const defaultDnsTimeoutMs = Math.max(250, Number(env("DIZZY_TOOL_DNS_TIMEOUT_MS", "2000")) || 2000);
  const dnsTimeoutMs = Number.isFinite(requestedDnsTimeoutMs) && requestedDnsTimeoutMs > 0
    ? Math.max(1, Math.floor(requestedDnsTimeoutMs))
    : defaultDnsTimeoutMs;
...
async function httpGet(url, timeoutMs = 15000) {
  ...
  const absoluteTimeoutMs = Math.max(50, timeoutMs);
  const deadline = Date.now() + absoluteTimeoutMs;
  function remainingBudget() {
    const remaining = deadline - Date.now();
    if (remaining <= 0) {
      throw new Error(`Request absolute timeout after ${absoluteTimeoutMs}ms`);
    }
    return remaining;
  }

  let currentTarget = await validateExternalUrl(url, { dnsTimeoutMs: remainingBudget() });

  for (let hop = 0; hop <= maxRedirects; hop += 1) {
    const res = await requestPinnedUrl(currentTarget, remainingBudget());
    if (res.status >= 300 && res.status < 400) {
      ...
      currentTarget = await validateExternalUrl(
        resolveRedirectUrl(currentTarget.url, Array.isArray(location) ? location[0] : location),
        { dnsTimeoutMs: remainingBudget() },
      );
      continue;
    }
...
async function runToolJobUnchecked(job) {
  const tool = job.tool;
  const p = job.payload ?? {};
...
export async function runToolJob(job, opts = {}) {
  return runWithToolLifecycle(job, runToolJobUnchecked, {
    lifecycleHooks: opts.lifecycleHooks,
    actorId: opts.actorId || "tool_runner",
    sessionId: opts.sessionId || `tool_${job?.id || "inline"}`,
  });
}
