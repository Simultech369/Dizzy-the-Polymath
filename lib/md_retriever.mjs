...
export function getMarkdownSourceSignature(opts = {}) {
  const trustZone = String(opts.trustZone || "").trim();
  const index = getIndex();
  const sources = index.docs
    .filter((doc) => !isRevoked(doc.frontmatter) && zoneAllows(doc.frontmatter, trustZone))
    .map((doc) => ({
      path: doc.relPath,
      source_hash: doc.sourceHash,
      kind: doc.kind,
      memory_status: String(doc.frontmatter?.memory_status || "active"),
      zone_allowed: String(doc.frontmatter?.zone_allowed || ""),
      source: String(doc.frontmatter?.source || "unknown"),
      confidence: String(doc.frontmatter?.confidence || ""),
      memory_class: String(doc.frontmatter?.memory_class || ""),
      last_reviewed: String(doc.frontmatter?.last_reviewed || ""),
      captured_at: String(doc.frontmatter?.captured_at || ""),
    }))
    .sort((a, b) => a.path.localeCompare(b.path));
  const digest = sha256Hex(JSON.stringify({
    schema: "dizzy.markdown_source_signature.v1",
  ...
  return {
    ...
    index_built_at_ms: index.builtAt,
  };
}
