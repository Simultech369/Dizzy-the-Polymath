...
function calculateDecay(doc) {
  ...
  const ageInDays = Number.isNaN(parsed)
    ? null
    : Math.max(0, (Date.now() - parsed) / (1000 * 60 * 60 * 24));
...
function scoreDoc(doc, qTokens, qSignals, idf, avgdl) {
  ...
  const decay = calculateDecay(doc);
...
export function getRelevantMarkdownSnippets(query, opts = {}) {
  ...
  for (const d of index.docs) {
    if (isRevoked(d.frontmatter)) continue;
    if (!zoneAllows(d.frontmatter, trustZone)) continue;
