...
async function writeRememberedMemory({
  ...
  recordKnownGoodTrajectory({
    convoKey,
    kind: "memory_write",
    summary: cleaned,
    why: "A memory write succeeded after experiential compression.",
    source: mode,
  });
...
function recordKnownGoodTrajectory({ convoKey, kind, summary, why, source = "runtime" }) {
  const cleanedSummary = sanitizeForMemory(summary).trim();
  if (!cleanedSummary) return null;
  const filePath = path.resolve(process.cwd(), "runtime", "known_good_trajectories.jsonl");
  appendJsonl(filePath, {
    t: new Date().toISOString(),
    convo_key: String(convoKey || "").slice(0, 120),
    kind: String(kind || "unknown").slice(0, 80),
    source: String(source || "runtime").slice(0, 80),
    summary: cleanedSummary.slice(0, 1200),
    why: sanitizeForMemory(why || "").trim().slice(0, 1200),
  });
  return filePath;
}
  ...
  if (text === "/remember" || text.toLowerCase().startsWith("/remember ")) {
    ...
    try {
      ...
      recordKnownGoodTrajectory({
        ...
        summary: cleaned,
        why: "Manual remember completed with experiential compression.",
