...
async function maybeChat(msg, { signal } = {}) {
      ...
      invokeRoute: async ({ route: selectedRoute }) => {
        if (routeBackend === "gemini") {
          ...
          return {
            ...
            reported_model: selectedRoute.model_id,
            response_kind: "text",
          };
        }

        if (routeBackend === "openai_compat" || routeBackend === "openrouter") {
          const text = await openaiCompatGenerateText({
            baseUrl: activeUrl,
  ...
  const systemPrompt = `${baseSystemPrompt}${trustZoneBlock}${skillBlock}${assembledContextBlock}`;
