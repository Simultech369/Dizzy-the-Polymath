...
async function maybeChat(msg, { signal } = {}) {
  ...
  if (retrievalAllowed) {
    const snapshot = createSourceSnapshot(process.cwd());
    const assemblerOutput = assembleContext({
      trust_zone: trustZone,
      task: text,
      budget_tokens: parseIntEnv("DIZZY_CONTEXT_BUDGET", 150000), // Reserve 50k for system prompts, budget 150k for context
