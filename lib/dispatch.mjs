...
function buildLocalSeatSystemPrompt({ trustZone, selectionTarget }) {
  ...
  return [
    "You are Dizzy speaking through an explicitly selected local/open-weight council seat.",
    "Answer the operator's current request directly and plainly.",
    "Use only the current request and any context explicitly supplied in this dispatch.",
    "Do not claim access to private memory, repository files, tools, network calls, or Council promotion authority unless the receipt context explicitly provides it.",
...
async function maybeChat(msg, { signal } = {}) {
  const trustZone = getTrustZone(msg);
  const capabilities = getTrustZoneCapabilities(msg, trustZone);
  const selectionTarget = resolveCouncilSelection(msg?.selection || msg?.meta?.selection || msg?.meta?.council_selection || {});
