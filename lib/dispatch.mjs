...
function buildDegradedModeReply(msg) {
  ...
  const convoPath = path.relative(
    process.cwd(),
    path.resolve(conversationDir(), `${convoKey}.jsonl`),
  ).replace(/\\/g, "/");
...
async function maybeChat(msg) {
  ...
  const convoPath = path.resolve(conversationDir(), `${convoKey}.jsonl`);
  ...
  if (text.toLowerCase().startsWith("/friction add ")) {
    ...
    try {
      ...
      const { entry, filePath } = appendFriction(parsed);
  ...
  if (text === "/trajectory" || text === "/trajectory help") {
    return {
      ...
      text: enforceOptionalityQuestion([
        "Trajectory commands:",
        "- `/trajectory list`",
        "- `/trajectory add {\"goal\":\"...\",\"success_criteria\":\"...\",\"actions_taken\":[\"...\"],\"outcome\":\"success\",\"reusable_pattern\":\"...\",\"reuse_tags\":[\"refinement\"],\"strength\":7}`",
  ...
  if (text === "/trajectory distill" || text.toLowerCase().startsWith("/trajectory distill ")) {
    ...
    try {
      ...
      return { kind: "reply", text: enforceOptionalityQuestion(`Trajectory distill failed: ${msgTxt}${body}`) };
    }
  }
  ...
  if (text.toLowerCase().startsWith("/trajectory add ")) {
    ...
    try {
      ...
      const { trajectory, filePath } = appendTrajectory(parsed);
      const rel = path.relative(process.cwd(), filePath).replace(/\\/g, "/");
      return {
        kind: "reply",
        text: enforceOptionalityQuestion([
  ...
  const systemPrompt = `${baseSystemPrompt}${trustZoneBlock}${ragBlock}${graphBlock}${trajectoryBlock}`;
