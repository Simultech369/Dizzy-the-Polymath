...
import { getGeminiModelForRoute, getModelRoute, getOpenAICompatModelForRoute } from "./model_router.mjs";
...
export function buildCapabilityReceipt(msg, audit = {}) {
  ...
  const retrievedFiles = Array.isArray(audit.retrieved_files)
  ...
  return {
    ...
    skills: {
      allowed: trustZone === "private_self" || trustZone === "trusted_collaborator",
    ...
    retrieval_audit: {
      ...
      rag: {
        ...
        count: Number(retrievalAudit.rag?.count ?? 0) || 0,
        files: Array.isArray(retrievalAudit.rag?.files) ? retrievalAudit.rag.files.map(String).filter(Boolean).sort() : [],
      },
      memory_graph: {
        attempted: capabilities.repo_retrieval_allowed,
        count: Number(retrievalAudit.memory_graph?.count ?? 0) || 0,
        files: Array.isArray(retrievalAudit.memory_graph?.files) ? retrievalAudit.memory_graph.files.map(String).filter(Boolean).sort() : [],
      },
      trajectories: {
        attempted: capabilities.repo_retrieval_allowed,
        count: Number(retrievalAudit.trajectories?.count ?? 0) || 0,
        ids: Array.isArray(retrievalAudit.trajectories?.ids) ? retrievalAudit.trajectories.ids.map(String).filter(Boolean).sort() : [],
      },
...
function fallbackUsagePath(convoKey) {
  return conversationArtifactPath(path.resolve(process.cwd(), "runtime", "fallback_usage"), convoKey, ".jsonl");
}
...
async function maybeChat(msg) {
  ...
  const { systemPrompt: baseSystemPrompt } = getCachedChatSystemPrompt({ trustZone: getTrustZone(msg) });
  ...
  if (text === "/friction list") {
    const rows = readFrictionEntries({ maxRows: Math.max(1, Number(env("DIZZY_FRICTION_LIST_LIMIT", "8")) || 8) }).slice(-8).reverse();
  ...
  if (text.toLowerCase().startsWith("/friction add ")) {
    ...
    try {
      ...
      const { entry, filePath } = appendFriction(parsed, { trustZone: getTrustZone(msg) });
  ...
  if (text === "/trajectory distill" || text.toLowerCase().startsWith("/trajectory distill ")) {
    ...
    try {
      ...
      if (json?.skip) {
        return { kind: "reply", text: enforceOptionalityQuestion(`Trajectory distill skipped: ${String(json.reason || "no reusable pattern found")}`) };
      }
      ...
      return { kind: "reply", text: enforceOptionalityQuestion(`Trajectory distill failed: ${formatExternalError(e)}`) };
    }
  }
  ...
  async function generateText({ systemPrompt, messages, temperature, timeoutMs, maxTokens, role = "chat" } = {}) {
    ...
    if (routeBackend === "openai_compat" || routeBackend === "openrouter") {
      ...
      return openaiCompatGenerateText({
        ...
        apiKey: compatApiKey,
        model: getOpenAICompatModelForRoute(route) || compatModel,
        systemPrompt: systemPrompt || "",
        messages: messages || [],
  ...
  if (text === "/improve") {
    ...
    try {
      ...
      return { kind: "reply", text: enforceOptionalityQuestion(`Improve failed: ${formatExternalError(e)}`) };
    }
  }
  ...
  if (text === "/memory_review" || text.toLowerCase().startsWith("/memory_review ")) {
    ...
    try {
      ...
      return { kind: "reply", text: enforceOptionalityQuestion(`Memory review failed: ${formatExternalError(e)}`) };
    }
  }
  ...
  if (text === "/remember" || text.toLowerCase().startsWith("/remember ")) {
    ...
    try {
      ...
      return { kind: "reply", text: enforceOptionalityQuestion(`Remember failed: ${formatExternalError(e)}`) };
    }
  }
  ...
  const ragBlock = rag.length
    ? [
      ...
      ...rag.map((r) => [
        ...
        sanitizeUntrustedInput(r.excerpt).sanitized,
        "</retrieved_context>",
      ].filter(Boolean).join("\n")),
  ...
  const graphBlock = graphCtx.docs.length
    ? [
      ...
      ...graphCtx.docs.map((d) => [
        ...
        "<retrieved_context trust=\"untrusted_data\">",
        sanitizeUntrustedInput(d.excerpt).sanitized,
        "</retrieved_context>",
      ].filter(Boolean).join("\n")),
  ...
  if (backend === "gemini") {
    try {
      ...
      if (!ephemeralHistory) {
        appendJsonl(convoPath, { t: new Date().toISOString(), role: "assistant", text: finalText, backend: "gemini", model_route: getModelRoute("chat").log });
      }
      try {
      ...
      return attachCapabilityReceipt({ kind: "reply", text: finalText }, msg, retrievalAudit);
    } catch (e) {
      ...
      if (canFallback) {
        ...
        try {
          ...
          if (!ephemeralHistory) {
            appendJsonl(convoPath, { t: new Date().toISOString(), role: "assistant", text: finalText, backend: "openai_compat", model_route: "fallback:openai_compat:transient_primary_failure" });
          }
          return attachCapabilityReceipt({ kind: "reply", text: finalText }, msg, fallbackUseRag ? retrievalAudit : { retrieved_files: [] });
        } catch (fallbackErr) {
          return { kind: "reply", text: enforceOptionalityQuestion(`Gemini failed; fallback failed: ${formatExternalError(fallbackErr)}`) };
        }
      }

      return { kind: "reply", text: enforceOptionalityQuestion(`Gemini chat error: ${formatExternalError(e)}`) };
    }
  }
  ...
  if (backend === "openai_compat" || backend === "openrouter") {
    try {
      ...
      const finalText = enforceOptionalityQuestion(reply);
      if (!ephemeralHistory) {
        appendJsonl(convoPath, { t: new Date().toISOString(), role: "assistant", text: finalText, backend: "openai_compat", model_route: getModelRoute("chat").log });
      }
      try {
        await maybeHandleAutoRemember({
          generateText,
      ...
      return { kind: "reply", text: enforceOptionalityQuestion(`OpenAI-compat chat error: ${formatExternalError(e)}`) };
    }
  }
...
export async function handleIncomingMessage({ message, enqueue }) {
  ...
  if (routed.kind === "enqueue") {
    ...
    try {
      ...
      return attachCapabilityReceipt({ kind: "ack", job_id: id, text: enforceOptionalityQuestion(`Queued ${routed.tool}. Job: ${id}`) }, message);
    } catch (e) {
