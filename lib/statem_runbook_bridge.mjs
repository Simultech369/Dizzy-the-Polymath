...
export async function executeStateMFsm({
  ...
  if (jobId && checkpointEngine) {
    const cp = checkpointEngine.readLatestCheckpoint(jobId);
    if (cp && cp.schema_version === "dizzy.statem_checkpoint.v1" && cp.runbook_name === runbookName) {
      currentState = cp.state;
      verificationAttempts = cp.verification_attempts || 0;
      verificationPassed = cp.verification_passed || false;
      transitions = cp.transitions || [];
      if (transitions.length > 0) startedAt = transitions[0].timestamp;
    }
  }

  const saveCheckpoint = () => {
    if (jobId && checkpointEngine) {
      checkpointEngine.saveCheckpoint(jobId, runbookName, {
        currentState,
  ...
  if (cancelIntent && currentState !== "COMPLETED" && currentState !== "CANCELLED") {
     currentState = "rollback";
  }

  try {
    if (currentState === "rollback") {
      transitions.push({ from: "SUSPEND", to: "rollback", timestamp: now().toISOString() });
      saveCheckpoint();
      const rollbackResult = rollbackHandler ? await rollbackHandler({ verificationAttempts }) : { ok: true };
      if (rollbackResult && rollbackResult.ok === false) {
        throw new Error(`Rollback phase failed: ${rollbackResult.error || "rollback error"}`);
      }
      transitions.push({ from: "rollback", to: "CANCELLED", timestamp: now().toISOString() });
    ...
    if (currentState === "execute" || currentState === "verify") {
    ...
    if (err.message === "SUSPEND") {
      // Just halt execution without transitioning to FAILED
      error = "Suspended";
    } else {
      error = err.message;
  ...
  const completedAt = now().toISOString();
  let finalStatus = executionSuccess ? "PASSED" : "FAILED";
  if (currentState === "CANCELLED") finalStatus = "CANCELLED";
