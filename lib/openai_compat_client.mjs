import { isRemoteCloudBackend } from "./model_router.mjs";
...
export async function openaiCompatGenerateText({
  ...
  let m = String(model || env("OPENAI_COMPAT_MODEL", "")).trim();
  if (m === "qwen/qwen3-32b" || m === "qwen3-32b") {
    m = "qwen/qwen-2.5-coder-32b-instruct";
  }
  if (!url) {
  ...
  try {
    ...
    if (err.name === "AbortError") {
      const timeoutErr = new Error(`OpenAI-compat provider timed out after ${timeoutMs}ms`);
