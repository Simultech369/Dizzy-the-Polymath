...
export function registerDashboardRoutes(app, options) {
  const { authToken, normalizeIp, isLoopbackHost, renderHtml } = options;
  if (typeof renderHtml !== "function") throw new Error("Dashboard HTML renderer is unavailable");
  ...
  app.get("/api/dashboard-data", guard, (req, res) => {
    try {
  ...
  app.get("/api/dashboard-query", guard, (req, res) => {
    try {
