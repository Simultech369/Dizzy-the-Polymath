...
<form id="dashboard-login-form" method="post" action="/dashboard/session" autocomplete="off"><label for="dashboard-token">Operator token</label> <input id="dashboard-token" name="token" type="password" required autocomplete="off" aria-describedby="login-error"> <button id="dashboard-login-submit" type="submit">Start session</button></form><p id="login-error" role="alert" aria-live="polite"></p>
