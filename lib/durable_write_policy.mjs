...
const SECRET_PATTERNS = [
  ...
  [/\b([A-Z0-9_]{2,64}(?:API|TOKEN|SECRET|KEY|PASSWORD)[A-Z0-9_]{0,64})(\s*=\s*)(\\*["'`]?)([^\s\\"'`;]{6,})(\3)/gi, (_m, key, sep, quote, _value, close) => `${key}${sep}${quote}[REDACTED]${close}`],
...
function hasStructuredSecretMaterial(value, key = "", depth = 0, seen = new WeakSet()) {
  if (key && isSensitiveSecretKey(key)) return true;
  if (value == null || depth > 12) return false;

  if (typeof value === "string") {
    if (redactSecretMaterial(value) !== value) return true;
    const trimmed = value.trim();
    if (!/^[\[{]/.test(trimmed)) return false;
    try {
      return hasStructuredSecretMaterial(JSON.parse(trimmed), "", depth + 1, seen);
    } catch {
      return false;
    }
  }
  ...
  if (Array.isArray(value)) {
    return value.some((item) => hasStructuredSecretMaterial(item, "", depth + 1, seen));
  }

  return Object.entries(value)
    .some(([childKey, childValue]) => hasStructuredSecretMaterial(childValue, childKey, depth + 1, seen));
}
...
export function assessDurableWrite(input = {}) {
  ...
  if (hasStructuredSecretMaterial(input.payload) || redactSecretMaterial(raw) !== raw) {
