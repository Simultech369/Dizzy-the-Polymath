...
function normalizeBindingList(values) {
  if (!Array.isArray(values)) return null;
  const normalized = [];
  const seen = new Set();
  for (const value of values) {
    const token = normalizeBindingText(value);
...
export function collectBridgeResponseErrors(response, request = null) {
  ...
  if (isPlainObject(request)) {
    const expectedRequestId = normalizeBindingText(request.request_id);
    const actualRequestId = normalizeBindingText(response.request_id);
    if (!actualRequestId) {
      addError(errors, "MISSING_RESPONSE_REQUEST_ID", "$.request_id", "response must include request_id");
    } else if (expectedRequestId && actualRequestId !== expectedRequestId) {
      addError(errors, "RESPONSE_REQUEST_ID_MISMATCH", "$.request_id", "response request_id must match original request_id");
    }

    const expectedTargetFiles = normalizeBindingList(request?.payload?.bounty_task?.target_files);
    const actualTargetFiles = normalizeBindingList(response.sanitized_target_files);
    if (expectedTargetFiles && !actualTargetFiles) {
      addError(errors, "MISSING_RESPONSE_TARGET_FILES", "$.sanitized_target_files", "response must include sanitized_target_files");
    } else if (expectedTargetFiles && actualTargetFiles && actualTargetFiles.join("\u0000") !== expectedTargetFiles.join("\u0000")) {
      addError(errors, "RESPONSE_TARGET_FILES_MISMATCH", "$.sanitized_target_files", "response sanitized_target_files must match original target_files");
    }

    const expectedVerificationCommand = normalizeBindingText(request?.payload?.bounty_task?.test_command);
    const actualVerificationCommand = normalizeBindingText(response.enforced_verification_command);
    if (expectedVerificationCommand && !actualVerificationCommand) {
      addError(errors, "MISSING_RESPONSE_VERIFICATION_COMMAND", "$.enforced_verification_command", "response must include enforced_verification_command");
    } else if (expectedVerificationCommand && actualVerificationCommand !== expectedVerificationCommand) {
      addError(errors, "RESPONSE_VERIFICATION_COMMAND_MISMATCH", "$.enforced_verification_command", "response enforced_verification_command must match original test_command");
    }
  }
