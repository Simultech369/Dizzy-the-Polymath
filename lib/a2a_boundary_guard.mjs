...
const PROMPT_MARKER_REPLACEMENTS = Object.freeze([
  [/<\|im_start\|>/gi, ""],
  [/<\|im_end\|>/gi, ""],
  [/<\|system\|>/gi, ""],
  [/<\|user\|>/gi, ""],
...
function resolveTrustStoreEntryConfig(value) {
  if (!isPlainObject(value)) {
    return { keyMaterial: value, metadata: {} };
  }
...
export class Ed25519TrustStore {
  constructor(initialKeys = {}) {
    ...
    if (initialKeys && typeof initialKeys === "object") {
      ...
      for (const [keyId, keyVal] of entries) {
        const { keyMaterial, metadata } = resolveTrustStoreEntryConfig(keyVal);
        this.addKey(keyId, keyMaterial, metadata);
      }
    }
  }
  ...
  getEntry(keyId) {
    return this._keys.get(String(keyId ?? "").trim()) ?? null;
  }

  getMetadata(keyId) {
    return this.getEntry(keyId)?.metadata ?? null;
  }
...
export function sanitizePromptInjection(payload, depth = 0) {
  ...
  if (typeof payload === "string") {
    ...
    for (let i = 0; i < 4; i++) {
      let next = current;
      for (const [pattern, replacement] of PROMPT_MARKER_REPLACEMENTS) {
    ...
    if (probe !== current) {
      throw new Error("Unstable prompt marker sanitization");
    }
    return current;
  }
  ...
  if (payload && typeof payload === "object") {
    ...
    for (const key of Object.keys(payload)) {
      ...
      const sanitizedKey = sanitizePromptInjection(key, depth + 1);
      if (!sanitizedKey) continue;
      if (isDangerousObjectKey(sanitizedKey)) {
        throw new Error("Dangerous key detected in A2A payload");
      }
      if (Object.prototype.hasOwnProperty.call(clean, sanitizedKey)) {
        throw new Error("Sanitized key collision detected in A2A payload");
      }
      clean[sanitizedKey] = sanitizePromptInjection(payload[key], depth + 1);
    }
    return clean;
  }
  return payload;
...
function nonceExpiresAt(value, maxTimestampAgeMs) {
  if (value && typeof value === "object" && Number.isFinite(value.expiresAt)) {
    return value.expiresAt;
  }
  const seenAt = Number(value);
  return Number.isFinite(seenAt) ? seenAt + maxTimestampAgeMs : 0;
}

function pruneExpiredNonces(cache, now, maxTimestampAgeMs) {
  for (const [key, seenAt] of cache.entries()) {
    if (now > nonceExpiresAt(seenAt, maxTimestampAgeMs)) cache.delete(key);
  }
}

function rememberNonce(cache, nonce, now, maxTimestampAgeMs, maxNonceCacheEntries, signedTimestampMs = now) {
  pruneExpiredNonces(cache, now, maxTimestampAgeMs);
  ...
  cache.set(nonce, {
    seenAt: now,
    signedTimestampMs,
    expiresAt: Math.max(now, signedTimestampMs) + maxTimestampAgeMs,
  });
  return true;
}
...
export function a2aBoundaryGuard(secretOrOptions, explicitOptions = {}) {
  ...
  return function (req, res, next) {
    try {
      ...
      pruneExpiredNonces(nonceCache, now, maxTimestampAgeMs);
      ...
      // Commit the nonce only after a supported signature path succeeds.
      rememberNonce(nonceCache, nonce, now, maxTimestampAgeMs, maxNonceCacheEntries, timestamp);

      if (req.body && typeof req.body === "object") {
        req.a2aSanitizedBody = sanitizePromptInjection(req.body);
      }
