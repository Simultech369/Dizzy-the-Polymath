...
function reviewerAttempts(stats = {}) {
  const runs = safeNumber(stats.runs, 0);
  const skipped = safeNumber(stats.skipped, 0);
  const failures = safeNumber(stats.failures, 0);
  return Math.max(runs + skipped + failures, safeNumber(stats.attempts, 0));
}

function reviewerAvailabilityStatus(stats = {}) {
  const runs = safeNumber(stats.runs, 0);
  const skipped = safeNumber(stats.skipped, 0);
  const failures = safeNumber(stats.failures, 0);
  const attempts = reviewerAttempts(stats);
  if (runs > 0) return "usable";
  if (attempts > 0 && skipped + failures > 0) return "unavailable_or_blocked";
  return "untried";
}
...
export function buildReviewCoverageReport({
  ...
  const reviewers = getReviewerRoster().map((reviewer) => {
    ...
    return {
      ...
      skipped: safeNumber(stats.skipped, 0),
      ...
      last_skipped_reason: stats.last_skipped_reason || "",
      last_likely_root_cause: stats.last_likely_root_cause || "",
      availability_status: reviewerAvailabilityStatus(stats),
  ...
  const reviewerAttempted = reviewers.filter((reviewer) => reviewer.attempts > 0);
  const reviewerUnavailable = reviewers.filter((reviewer) => reviewer.availability_status === "unavailable_or_blocked");
  ...
  return {
    ...
    reviewers: {
      ...
      attempted: reviewerAttempted.length,
      touched: reviewerTouched.length,
      untouched: reviewers.length - reviewerTouched.length,
      attempted_pct: pct(reviewerAttempted.length, reviewers.length),
      touched_pct: pct(reviewerTouched.length, reviewers.length),
      next_rotation: reviewers
        .filter((reviewer) => reviewer.runs === 0 && reviewer.attempts === 0)
        .sort((a, b) => a.role_key.localeCompare(b.role_key))
        .slice(0, max),
      availability_recheck: reviewerUnavailable
        .sort((a, b) => b.attempts - a.attempts || a.role_key.localeCompare(b.role_key))
        .slice(0, max),
