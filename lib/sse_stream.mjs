...
export class StreamingLatencyTracker {
  constructor({
    ...
    this.stallThresholdMs = Math.max(100, Math.floor(Number(stallThresholdMs) || 5000));
    this.stallAbortMs = Math.max(this.stallThresholdMs, Math.floor(Number(stallAbortMs) || 30000));
    this.frameCount = 0;
    this.firstFrameAtMs = null;
    this.lastFrameAtMs = null;
    this.maxItlMs = null;
    this.totalItlMs = 0;
    this.itlIntervalsCount = 0;
    this.stallCount = 0;
  }

  recordFrame(nowMs = Date.now()) {
    const ts = Number(nowMs);
    const safeTs = Number.isFinite(ts) ? ts : Date.now();
    this.frameCount += 1;
    if (this.lastFrameAtMs === null) {
      this.firstFrameAtMs = safeTs;
      this.lastFrameAtMs = safeTs;
      return { ok: true, itl_ms: 0, stalled: false };
    }
    const itl = Math.max(0, safeTs - this.lastFrameAtMs);
    this.lastFrameAtMs = safeTs;
    this.itlIntervalsCount += 1;
    this.totalItlMs += itl;
    if (this.maxItlMs === null || itl > this.maxItlMs) {
      this.maxItlMs = itl;
    }
    const stalled = itl >= this.stallThresholdMs;
    if (stalled) {
      this.stallCount += 1;
    }
    if (itl >= this.stallAbortMs) {
      return {
        ok: false,
        status: "stream_stall_timeout",
    ...
    return { ok: true, itl_ms: itl, stalled };
  }

  getMetrics() {
    const avgItl = this.itlIntervalsCount > 0
      ? Math.round(this.totalItlMs / this.itlIntervalsCount)
      : null;
    return {
      frame_count: this.frameCount,
      max_itl_ms: this.maxItlMs,
      avg_itl_ms: avgItl,
...
export function buildStreamReceipt({
  ...
  return {
    ...
    max_itl_ms: Number.isFinite(Number(maxItlMs)) ? Math.max(0, Math.floor(Number(maxItlMs))) : null,
    avg_itl_ms: Number.isFinite(Number(avgItlMs)) ? Math.max(0, Math.floor(Number(avgItlMs))) : null,
    stall_count: Math.max(0, Math.floor(Number(stallCount) || 0)),
