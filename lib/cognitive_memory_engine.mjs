...
export class CognitiveMemoryEngine {
  ...
  capture(input = {}) {
    ...
    const conflicts = activeSamePartition.filter((m) => Number(m.polarity || 0) !== 0 && polarity !== 0 && Number(m.polarity) !== polarity);
      if (conflicts.length) {
                // [W-0126] Deterministic Policy Scorer (Replaces Z3 Python Bridge)
        const MARGIN = 0.05;
        let incomingScore = (Number(input.confidence) || 0.7) + (Number(input.reinforcement_count || 1) * 0.02);
        
        // Find highest existing score
        let highestExistingScore = -1;
        let highestExistingId = null;
        for (const m of conflicts) {
          const score = (Number(m.confidence) || 0.5) + (Number(m.reinforcement_count || 1) * 0.02);
