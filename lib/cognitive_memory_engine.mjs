...
export const VALID_SENSITIVITY_TIERS = new Set(["public_safe", "normal", "do_not_export"]);
...
function normalizeTrustZone(value) {
  return String(value || "private_self").trim().toLowerCase();
}

function normalizeSensitivityTier(value) {
...
function normalizePagePath(value) {
  ...
  if (!relPath.startsWith("entries/") || !relPath.endsWith(".md")) return "";
  return relPath;
}

function memoryPartitionKeyFromParts(canonicalKey, trustZone, sensitivityTier) {
  return [
    String(canonicalKey || "").trim().toLowerCase(),
    normalizeTrustZone(trustZone),
    normalizeSensitivityTier(sensitivityTier),
  ].join("|");
}

function memoryPartitionKey(memory) {
  return memoryPartitionKeyFromParts(memory.canonical_key, memory.trust_zone, memory.sensitivity_tier);
}

function sameMemoryPartition(memory, { canonicalKey, trustZone, sensitivityTier }) {
  return memoryPartitionKey(memory) === memoryPartitionKeyFromParts(canonicalKey, trustZone, sensitivityTier);
}

function memoryPageRelPath(memory) {
  return normalizePagePath(memory.page_path) || `entries/${slugify(memory.canonical_key)}.md`;
}

function partitionedMemoryPageRelPath(memory) {
  return `entries/${slugify(memory.canonical_key)}--${slugify(memory.trust_zone)}--${slugify(memory.sensitivity_tier)}.md`;
}

function uniqueMemoryPageRelPath(memory, index) {
  const idPart = slugify(memory.memory_id || `memory-${index}`).slice(0, 48) || `memory-${index}`;
  return `entries/${slugify(memory.canonical_key)}--${slugify(memory.trust_zone)}--${slugify(memory.sensitivity_tier)}--${idPart}.md`;
}

function assignMemoryPagePaths(memories = []) {
  const used = new Set();
  for (let i = 0; i < memories.length; i += 1) {
    const memory = memories[i];
    const current = normalizePagePath(memory.page_path);
    const candidates = unique([
      current,
      `entries/${slugify(memory.canonical_key)}.md`,
      partitionedMemoryPageRelPath(memory),
      uniqueMemoryPageRelPath(memory, i),
    ].filter(Boolean));
    let selected = candidates.find((candidate) => !used.has(candidate));
    if (!selected) {
      let suffix = 2;
      do {
        selected = uniqueMemoryPageRelPath({ ...memory, memory_id: `${memory.memory_id || i}-${suffix}` }, i);
        suffix += 1;
      } while (used.has(selected));
    }
    memory.page_path = selected;
    used.add(selected);
  }
  return memories;
}

function chooseMemoryPageRelPath({ canonicalKey, trustZone, sensitivityTier }, memories = []) {
  const basePath = `entries/${slugify(canonicalKey)}.md`;
  const partitionKey = memoryPartitionKeyFromParts(canonicalKey, trustZone, sensitivityTier);
  const basePathOwner = memories.find((memory) => memoryPageRelPath(memory).replace(/\\/g, "/") === basePath);
  if (!basePathOwner || memoryPartitionKey(basePathOwner) === partitionKey) return basePath;
  return `entries/${slugify(canonicalKey)}--${slugify(trustZone)}--${slugify(sensitivityTier)}.md`;
}
...
function memoryToMarkdown(memory) {
  ...
  return [
    ...
    `trust_zone: ${memory.trust_zone || "private_self"}`,
    `sensitivity_tier: ${memory.sensitivity_tier || "normal"}`,
    `status: ${memory.status || "active"}`,
    `confidence: ${memory.confidence || 0}`,
    `reinforcement_count: ${memory.reinforcement_count || 1}`,
    `polarity: ${memory.polarity || 0}`,
...
function extractMetadata(markdown) {
  ...
  for (const line of lines) {
    ...
    if (colonIdx > -1) {
      ...
      if (key === "confidence" || key === "polarity") metadata[key] = parseFloat(value);
      else if (key === "reinforcement_count") metadata[key] = parseInt(value, 10);
...
function buildSchemaMarkdown(timestamp) {
  return [
    ...
    "## Directories",
    "",
    "- `entries/`: compiled memory pages, one canonical key per page unless trust-zone or sensitivity partitions require separate pages.",
...
function writeWiki(wikiRootPath, memories, timestamp) {
  if (!wikiRootPath) return;
  assignMemoryPagePaths(memories);
  const entriesDir = path.join(wikiRootPath, "entries");
...
function trustZoneAllows(memory, trustZone) {
  ...
  const sensitivity = normalizeSensitivityTier(memory.sensitivity_tier);
...
export function classifyForCapture(input = {}) {
  ...
  const rawSensitivity = input.sensitivityTier || input.sensitivity_tier;
  const sensitivityTier = normalizeSensitivityTier(rawSensitivity);
  if ((rawSensitivity !== undefined && rawSensitivity !== null && String(rawSensitivity).trim() !== "") && !VALID_SENSITIVITY_TIERS.has(sensitivityTier)) {
    return { decision: "reject", reason: "invalid_sensitivity_tier", memory_class: "" };
  }
...
export class CognitiveMemoryEngine {
  constructor(opts = {}) {
    ...
    assignMemoryPagePaths(this.memories);
  }
  ...
  capture(input = {}) {
    ...
    const trustZone = normalizeTrustZone(input.trustZone || input.trust_zone);
    const sensitivityTier = normalizeSensitivityTier(input.sensitivityTier || input.sensitivity_tier);
    const activeSamePartition = this.memories.filter((m) => (
      m.status === "active"
      && m.canonical_key === canonicalKey
      && sameMemoryPartition(m, { canonicalKey, trustZone, sensitivityTier })
    ));
    const conflicts = activeSamePartition.filter((m) => Number(m.polarity || 0) !== 0 && polarity !== 0 && Number(m.polarity) !== polarity);
      if (conflicts.length) {
        // [Governance Predicate] Hard rules cannot be automatically superseded by empirical scores
        const hasDurableRule = conflicts.some(m => String(m.memory_class).toLowerCase() === "durable_rule");
        if (hasDurableRule) {
          const receipt = makeReceipt("reconcile", {
            timestamp,
            status: "rejected_governance_invariant",
            canonical_key: canonicalKey
          });
          return { decision: "reject", reason: "rejected_governance_invariant", receipt };
        }

        // [W-0126] Deterministic Policy Scorer
        const clamp01 = (v) => Math.max(0, Math.min(1, v));
        const MARGIN = 0.05;
        const incomingConfidence = input.confidence !== undefined ? clamp01(Number(input.confidence)) : 0.7;
        const incomingReinforcement = Math.min(Number(input.reinforcement_count || 1), 50);
        let incomingScore = incomingConfidence + (incomingReinforcement * 0.02);
        
        // Find highest existing score
        let highestExistingScore = -1;
        let highestExistingId = null;
        for (const m of conflicts) {
          const mConf = m.confidence !== undefined ? clamp01(Number(m.confidence)) : 0.5;
          const mReinf = Math.min(Number(m.reinforcement_count || 1), 50);
          const score = mConf + (mReinf * 0.02);
          if (score > highestExistingScore) {
            highestExistingScore = score;
            highestExistingId = m.memory_id;
          }
        }

        let incomingWins = false;
        let existingWinnerId = null;
        
        if (incomingScore >= highestExistingScore + MARGIN) {
          incomingWins = true;
        } else if (highestExistingScore >= incomingScore + MARGIN) {
          existingWinnerId = highestExistingId;
        }

        if (incomingWins) {
          // INCOMING WINS: archive the existing conflicting memories
          for (const m of conflicts) {
            m.status = "archived"; m.archive_reason = "deterministic_superseded"; m.updated_at = timestamp;
            appendLogEntry(this.wikiRootPath, {
              timestamp,
              action: "decay",
              status: "deterministic_superseded",
              memory_id: m.memory_id
            });
          }
          // The incoming memory will be written below by the main capture flow.
        } else if (existingWinnerId) {
          // EXISTING WINS: reject incoming
          const receipt = makeReceipt("reconcile", {
            timestamp,
            status: "rejected_policy_violation",
            canonical_key: canonicalKey,
            winner_memory_id: existingWinnerId
          });
          return { decision: "reject", reason: "rejected_policy_violation", receipt };
        } else {
          // TIE OR WEAK MARGIN -> fail-closed flag_conflict
            const receipt = makeReceipt("reconcile", {
              timestamp,
    ...
    const duplicate = activeSamePartition.find((m) => {
    ...
    const memory = {
      ...
      page_path: chooseMemoryPageRelPath({ canonicalKey, trustZone, sensitivityTier }, this.memories),
    };
  ...
  consolidate({ now } = {}) {
    ...
    for (const memory of this.memories.filter((m) => m.status === "active")) {
      const key = memoryPartitionKey(memory);
      if (!byKey.has(key)) byKey.set(key, []);
      byKey.get(key).push(memory);
    }
  ...
  retrieve(query, opts = {}) {
    ...
    const rawZone = opts.trustZone || opts.trust_zone;
    const trustZone = normalizeTrustZone(rawZone);
    if (!VALID_TRUST_ZONES.has(trustZone)) {
      const receipt = makeReceipt("retrieve", {
        timestamp,
        status: "invalid_trust_zone",
        reason: `Invalid retrieval trust zone: ${rawZone || "empty"}`,
...
export function createA2AMemoryUpdateEnvelope({
  ...
  const safeTrustZone = normalizeTrustZone(trustZone || "trusted_collaborator");
