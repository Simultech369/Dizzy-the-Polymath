...
import { createBridgeRequest } from "./node_python_council_bridge_contract.mjs";
...
export class CognitiveMemoryEngine {
  ...
  capture(input = {}) {
    ...
    const conflicts = activeSamePartition.filter((m) => Number(m.polarity || 0) !== 0 && polarity !== 0 && Number(m.polarity) !== polarity);
      if (conflicts.length) {
        // [Neurosymbolic Proof Planner Integration]
        // Synchronously call the python bridge to resolve the contradiction formally.
        let winnerId = null;
        try {
          const req = createBridgeRequest({
            payload: {
              incoming_memory: {
                memory_id: "incoming_mem",
                confidence: Number(input.confidence) || 0.7,
                last_accessed_at: timestamp
              },
              existing_memories: conflicts.map(m => ({
          ...
          const scriptPath = path.resolve(process.cwd(), "sidecars/council_engine/memory_resolution_bridge.py");
          const bridgeResultRaw = execFileSync("python", [scriptPath], {
            input: JSON.stringify(req),
            encoding: "utf-8",
            timeout: 2500 // strict fail-closed timeout
          });
          
          const bridgeResult = JSON.parse(bridgeResultRaw);
          if (bridgeResult.status === "VERIFIED_DISPATCH" && bridgeResult.payload && bridgeResult.payload.winner_id) {
            winnerId = bridgeResult.payload.winner_id;
          }
        } catch (err) {
          // Fallback to flag_conflict on timeout/crash
          console.error("[CognitiveMemoryEngine] Python FormalProver bridge failed, falling back to flag_conflict:", err.message);
        }

        if (winnerId === "incoming_mem") {
          // INCOMING WINS: archive the existing conflicting memories
          for (const m of conflicts) {
            m.status = "archived"; m.archive_reason = "neurosymbolic_superseded"; m.updated_at = timestamp;
            appendLogEntry(this.wikiRootPath, {
              timestamp,
              action: "decay",
              status: "neurosymbolic_superseded",
              memory_id: m.memory_id
            });
          }
          ...
          const receipt = makeReceipt("reconcile", {
            timestamp,
            status: "rejected_invariant_violation",
            canonical_key: canonicalKey,
            winner_memory_id: winnerId
          });
          return { decision: "reject", reason: "rejected_invariant_violation", receipt };
        } else {
            // TIE / UNKNOWN / ERROR -> fail-closed flag_conflict
            const receipt = makeReceipt("reconcile", {
              timestamp,
