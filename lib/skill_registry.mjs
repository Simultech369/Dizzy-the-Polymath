...
export function discoverLocalSkills(options = {}) {
  ...
  return { version: manifest.version, reviewed_at: manifest.reviewed_at, default_max_selected: manifest.default_max_selected || 3, skills, issues };
}
...
export function selectLocalSkills(query, options = {}) {
  ...
  if (!ALLOWED_ZONES.has(trustZone)) {
    return { registry, selected: [], requested, rejected: requested.map((name) => ({ name, reason: "trust_zone_blocks_local_skills" })), mode: "blocked" };
  }

  const available = new Map(registry.skills.filter((skill) => skill.status === "active" || skill.status === "restricted").map((skill) => [skill.name, skill]));
  const rejected = [];
  let selected = [];
  let mode = "automatic";
  ...
  if (requested.length) {
    ...
    selected = [...available.values()].map((skill) => {
      const matched = skill.trigger_terms.filter((term) => haystack.includes(String(term).toLowerCase()));
  ...
  selected = selected.slice(0, maxSelected);
  return { registry, selected, requested, rejected, mode };
}
