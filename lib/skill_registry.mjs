...
function blockingIssue(message) {
  return { message, severity: "blocking" };
}
...
export function discoverLocalSkills(options = {}) {
  ...
  for (const entry of fs.readdirSync(rootDir, { withFileTypes: true })) {
    ...
    const { data, body, issues: frontmatterIssues = [] } = parseFrontmatter(raw);
    ...
    const addIssue = (issue) => {
      skillIssues.push(blockingIssue(issue));
      issues.push(`${entry.name}: ${issue}`);
    };

    for (const issue of frontmatterIssues) addIssue(issue);
    if (!name) addIssue("missing frontmatter name");
    if (name && name !== entry.name) addIssue(`frontmatter name '${name}' does not match directory`);
    if (!description) addIssue("missing frontmatter description");
    for (const key of Object.keys(data || {})) {
      if (!MANIFEST_V1_KEYS.has(key)) addIssue(`unknown frontmatter key '${key}'`);
    }
    if (!body.trim()) addIssue("empty skill body");
    if (!config) addIssue("missing registry disposition");
    ...
    skills.push({
      ...
      issues: skillIssues,
      manifest_valid: skillIssues.length === 0,
      body: body.trim(),
    });
  }
...
export function selectLocalSkills(query, options = {}) {
  ...
  for (const skill of selected) {
    if (!skill.manifest_valid) {
      rejected.push({
        name: skill.name,
        reason: "malformed_manifest",
        issues: Array.isArray(skill.issues) ? skill.issues.map((issue) => issue.message || String(issue)) : [],
      });
      continue;
    }
