...
function git(args, { rootDir = process.cwd() } = {}) {
  const result = spawnSync("git", args, { encoding: "utf8", cwd: rootDir, maxBuffer: 50 * 1024 * 1024 });
...
export function changedFilesFromWorktree({ rootDir = process.cwd() } = {}) {
  const files = [];
  for (const line of git(["status", "--short"], { rootDir }).split(/\r?\n/)) {
...
function summarizeSynthesis(synthesis = {}) {
  return {
    synthesis_id: synthesis.synthesis_id || "",
    proposed_state_transition: synthesis.proposed_state_transition || "",
    counts: synthesis.counts || {},
    agreement_clusters: Array.isArray(synthesis.agreement_clusters) ? synthesis.agreement_clusters.length : 0,
    disagreement_clusters: Array.isArray(synthesis.disagreement_clusters) ? synthesis.disagreement_clusters.length : 0,
    blocking_clusters: Array.isArray(synthesis.blocking_clusters) ? synthesis.blocking_clusters.length : 0,
    availability_rechecks: Array.isArray(synthesis.availability_rechecks) ? synthesis.availability_rechecks.length : 0,
    suggested_tests: Array.isArray(synthesis.suggested_tests) ? synthesis.suggested_tests : [],
    next_actions: Array.isArray(synthesis.next_actions) ? synthesis.next_actions : [],
    authority: synthesis.authority || "synthesis_is_triage_not_authority",
  };
}
...
export async function runReviewLoopSupervisor({
  ...
  reviewSynthesisOutPath = "reviews/review_synthesis_latest.json",
  ...
  if (writeReceipts) {
    ...
    receiptPaths.review_synthesis = writeReviewSynthesis(synthesis, { rootDir, outPath: reviewSynthesisOutPath });
  ...
  return {
    ...
    synthesis_summary: summarizeSynthesis(synthesis),
