...
function validateRouteResult(result, route) {
  ...
  const hasPayload = result.payload !== undefined && result.payload !== null;
...
function sealRoutingPlan(plan) {
  const sealed = { ...plan, routing_receipt: buildRoutingReceipt(plan) };
...
export async function executeRoutingPlan(plan, runtime = {}) {
  ...
  if (plan[ROUTING_PLAN_AUTHORITY] !== true) {
    return {
      schema: ROUTING_EXECUTION_SCHEMA,
      started_at: startedAt,
      status: "BLOCKED",
