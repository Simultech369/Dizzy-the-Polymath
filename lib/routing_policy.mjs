...
function payloadIsMeaningful(payload) {
  ...
  if (Array.isArray(payload)) return payload.some((item) => payloadIsMeaningful(item));
  if (isPlainObject(payload)) return Object.values(payload).some((item) => payloadIsMeaningful(item));
  return false;
}
