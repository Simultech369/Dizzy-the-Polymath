...
export const TIER_CACHED_INPUT_COST_PER_1K = Object.freeze({
  T0: 0.0,
  T1: 0.0000375, // 75% discount: $0.0375 / 1M tokens
  T2: 0.0005,    // 75% discount: $0.50 / 1M tokens
  T3: 0.00125,   // 75% discount: $1.25 / 1M tokens
});
...
export function calculateAsymmetricCostUsd({ tier = "T2", inputTokens = 0, outputTokens = 0, cachedTokens = 0 } = {}) {
  const inRate = TIER_INPUT_COST_PER_1K[tier] ?? 0.0020;
  const cachedRate = TIER_CACHED_INPUT_COST_PER_1K[tier] ?? (inRate * 0.25);
  const outRate = TIER_OUTPUT_COST_PER_1K[tier] ?? 0.0080;
  const totalIn = Math.max(0, Number(inputTokens) || 0);
  const cached = Math.max(0, Math.min(totalIn, Number(cachedTokens) || 0));
  const uncached = Math.max(0, totalIn - cached);
  const cost = (uncached / 1000) * inRate +
...
function validateRouteResult(result, route) {
  ...
  if (result.usage !== undefined && result.usage !== null && !isPlainObject(result.usage)) violations.push("usage_malformed");
  return violations;
}

export function parseRoutingUsage(usage) {
  if (!isPlainObject(usage)) return null;

  const hasInputTokens = "input_tokens" in usage;
  const hasOutputTokens = "output_tokens" in usage;

  const rawInput = hasInputTokens ? usage.input_tokens : usage.prompt_tokens;
  const rawOutput = hasOutputTokens ? usage.output_tokens : usage.completion_tokens;

  const cacheCandidates = [];
  if (usage.prompt_tokens_details && typeof usage.prompt_tokens_details === "object" && "cached_tokens" in usage.prompt_tokens_details) {
    cacheCandidates.push(usage.prompt_tokens_details.cached_tokens);
  }
  if ("cached_tokens" in usage) {
    cacheCandidates.push(usage.cached_tokens);
  }
  if ("prompt_cache_hit_tokens" in usage) {
    cacheCandidates.push(usage.prompt_cache_hit_tokens);
  }
  ...
  const inputTokens = parseUsageTokenCount(rawInput);
  const outputTokens = parseUsageTokenCount(rawOutput);
  ...
  let totalTokens = sum;
  const hasTotal = "total_tokens" in usage || "totalTokenCount" in usage;
  if (hasTotal) {
    const rawTotal = "total_tokens" in usage ? usage.total_tokens : usage.totalTokenCount;
    const parsedTotal = parseUsageTokenCount(rawTotal);
    if (parsedTotal === null || parsedTotal < sum) return null;
...
function hasCompleteUsage(usage) {
  return parseRoutingUsage(usage) !== null;
}
...
export async function executeRoutingPlan(plan, runtime = {}) {
  ...
  for (const route of chain.slice(0, maxAttempts)) {
    ...
    try {
      ...
      const parsedUsage = parseRoutingUsage(result?.usage);
      attempt.usage_known = parsedUsage !== null;
      ...
      if (attempt.usage_known && parsedUsage) {
  ...
  const baselineLatencyMs = TIER_BASELINE_LATENCY_MS[plan.selected_tier] ?? 1200;
  const providerInvoked = attempts.some((attempt) => attempt.adapter_invoked === true || attempt.transport_started === true || attempt.status === "FAILED" || attempt.status === "SUCCEEDED");
  const actualCostUsd = providerInvoked ? null : 0.0;
  const costDeltaUsd = actualCostUsd === null
    ? null
    : (plan.estimated_cost_usd !== null ? Number((actualCostUsd - plan.estimated_cost_usd).toFixed(6)) : 0.0);
  const routingDeltas = {
    actual_cost_usd: actualCostUsd,
    cost_delta_usd: costDeltaUsd,
    baseline_latency_ms: baselineLatencyMs,
