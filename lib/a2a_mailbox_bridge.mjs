...
function normalizeA2AScopeList(value) {
  if (value == null) return [];
  const values = Array.isArray(value) ? value : String(value).split(",");
  return [...new Set(values.map((item) => String(item).trim().toLowerCase()).filter(Boolean))];
}

function getA2ATrustStoreEntry(trustStore, keyId) {
  if (!trustStore) return null;
  if (typeof trustStore.getEntry === "function") {
    return trustStore.getEntry(keyId);
  }

  if (typeof trustStore.getKey !== "function") {
    return null;
  }

  const keyObject = trustStore.getKey(keyId);
  if (!keyObject) {
    return null;
  }

  const metadata = typeof trustStore.getMetadata === "function" ? trustStore.getMetadata(keyId) : {};
  return {
...
function resolveA2ASignerPolicy(metadata = {}) {
  const senderScopes = normalizeA2AScopeList(
    metadata.authorized_sender_ids ??
    metadata.authorizedSenderIds ??
    metadata.allowed_sender_ids ??
    metadata.allowedSenderIds ??
    metadata.sender_ids ??
    metadata.senderIds ??
    metadata.sender_id ??
    metadata.senderId ??
  ...
  const trustZoneScopes = normalizeA2AScopeList(
    metadata.authorized_trust_zones ??
    metadata.authorizedTrustZones ??
    metadata.allowed_trust_zones ??
    metadata.allowedTrustZones ??
    metadata.trust_zones ??
    metadata.trustZones ??
    metadata.trust_zone ??
    metadata.trustZone
  );

  return { senderScopes, trustZoneScopes };
}

function authorizeA2ASignerForMessage(trustStore, keyId, message) {
  const entry = getA2ATrustStoreEntry(trustStore, keyId);
  if (!entry) {
  ...
  const trustZone = String(message.trust_zone || "").trim().toLowerCase();
  const normalizedKeyId = String(keyId || "").trim().toLowerCase();
  const { senderScopes, trustZoneScopes } = resolveA2ASignerPolicy(entry.metadata || {});

  if (senderScopes.length > 0) {
    if (!senderScopes.includes(senderId)) {
      return { ok: false, reason: `Signer keyId '${keyId}' is not authorized for sender_id '${message.sender_id}'` };
    }
  } else if (normalizedKeyId !== senderId) {
    return { ok: false, reason: `Signer keyId '${keyId}' is not authorized for sender_id '${message.sender_id}'` };
  }

  if (trustZoneScopes.length > 0) {
    if (!trustZoneScopes.includes(trustZone)) {
      return { ok: false, reason: `Signer keyId '${keyId}' is not authorized for trust_zone '${message.trust_zone}'` };
    }
  } else if (!["private_self", "trusted_collaborator"].includes(trustZone)) {
    return { ok: false, reason: `Signer keyId '${keyId}' is not authorized for trust_zone '${message.trust_zone}'` };
  }
...
export function computeA2AEnvelopeSigningData(message, timestamp, nonce) {
  const canonicalFrame = {
    ...
    message: {
      ...
      trust_zone: message.trust_zone,
      priority: message.priority,
      parent_task_id: message.parent_task_id ?? null,
  ...
  return JSON.stringify(canonicalFrame);
}
...
export function verifyA2AMessageEnvelope(envelope, {
  ...
  if (algo === "ed25519") {
    ...
    const keyId = String(envelope.signer_key_id || "").trim();
    if (!keyId) {
      return { ok: false, reason: "Missing signer_key_id for Ed25519 envelope" };
    }
    const authorization = authorizeA2ASignerForMessage(trustStore, keyId, msg);
    if (!authorization.ok) {
      return authorization;
    }
    const pubKey = authorization.entry.keyObject;
    ...
    if (trustStore && typeof trustStore.size === "function" && trustStore.size() > 0) {
      return { ok: false, reason: "Algorithm 'hmac-sha256' is not permitted by policy when signer-authorized Ed25519 trustStore policy is configured" };
    }
    const sigHex = String(envelope.detached_signature || "").trim();
...
export class A2AMailboxQueue {
  ...
  _restoreMessageToQueue(messageId) {
    if (!this.messages.has(messageId)) {
      return false;
    }
    if (!this.queue.includes(messageId)) {
      this.queue.unshift(messageId);
    }
    return true;
  }
  ...
  acknowledge({ messageId, leaseToken, outcome = "PROCESSED" } = {}) {
    ...
    if (now > lease.expiresAt) {
      ...
      this._restoreMessageToQueue(messageId);
  ...
  pruneExpiredLeases() {
    ...
    for (const [msgId, lease] of this.leases.entries()) {
      if (now > lease.expiresAt) {
        ...
        if (this._restoreMessageToQueue(msgId)) {
          recoveredCount++;
        }
      }
    }
