...
function scrubExportValue(value, key = "", depth = 0) {
  ...
  if (typeof value === "string") {
    const redactedSystemValue = redactSecretMaterial(value);
    if (redactedSystemValue !== value) return { value: redactedSystemValue, did_redact: true };
    if (isPotentiallySensitive(value)) return { value: `[REDACTED_HIGH_ENTROPY_CONTENT]`, did_redact: true };
    return { value, did_redact: false };
  }
