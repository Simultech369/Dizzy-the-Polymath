...
/**
 * Assembles context into a flat packed string based on the given sources and budget.
 * 
 * Security Contract:
 * - This function enforces a narrow trust-zone contract: it blocks `do_not_export` records
 *   from leaking outside the `private_self` zone. 
 * - It does NOT enforce the full origin trust-zone matrix (e.g. cross-zone public bleed).
 * - Full matrix enforcement is assumed to be handled upstream by the retrieval allowed
 *   capabilities in the dispatch pipeline.
 */
export function assembleContext({
  ...
  // 1. Validate
  const budget = Number(budget_bytes) || 0;
  if (budget <= 0) throw new Error("budget_bytes must be > 0");
  ...
  for (const r of ordered) {
    ...
    if (current_bytes + renderedBytes <= budget) {
