...
To protect the integrity of exported data and prevent downstream systems from being compromised by malicious payloads, client-continuity exports pass string values through secret scrubbing and prompt-injection redaction. Retained paid/client continuity text is redacted before storage when it matches known prompt-injection patterns. Private/operator local conversation history remains a local working transcript, not a client export guarantee. This process is handled by `lib/janitor.mjs`; flagged payloads are redacted or blocked rather than echoed back inside the neutralization marker, so hostile text is not replayed into client-continuity exports or retained paid/client prompt history. (Verification command: `node scripts/fuzzing_and_injection_tests.mjs`)
